...
export function AreasCursosPage() {
  ...
  const handleCreateArea = async () => {
    ...
    try {
      await apiPost('/api/areas', { nombre: newAreaName.trim(), color: newAreaColor });
      setNewAreaName('');
  ...
  const handleEditArea = async () => {
    ...
    try {
      await apiPut(`/api/areas/${editingArea.id}`, { nombre: newAreaName.trim(), color: newAreaColor });
      setEditingArea(null);
      setNewAreaName('');
      setNewAreaColor('#2F5FD0');
      loadAreas();
  ...
  return (
    <>
      <div className="sec-header">
        ...
        <button type="button" className="btn-primary" onClick={() => setShowNewArea(true)}>
          + Nueva área
        </button>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowNewArea(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-actions">
              <button type="button" className="btn-secondary" onClick={() => setShowNewArea(false)}>Cancelar</button>
              <button type="button" className="btn-primary" onClick={handleCreateArea}>Crear</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingArea && (
        <div className="modal-overlay" onClick={() => setEditingArea(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-actions">
              <button type="button" className="btn-secondary" onClick={() => setEditingArea(null)}>Cancelar</button>
              <button type="button" className="btn-primary" onClick={handleEditArea}>Guardar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div key={sec.id} className="card" style={{ marginBottom: 12 }}>
          <div className="area-section-header">
            <div className="area-section-name">
              <span className="area-color-dot" style={{ background: sec.color }} />
              {sec.nombre}
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button type="button" className="ab" onClick={() => setAddingCursoTo(sec.id.toString())}>+ Curso</button>
              <button type="button" className="ab" onClick={() => { setEditingArea(sec); setNewAreaName(sec.nombre); setNewAreaColor(sec.color); }}>Editar área</button>
              <button type="button" className="ab" onClick={() => handleDeleteArea(sec.id)}>Eliminar área</button>
            </div>
          </div>
          {addingCursoTo === sec.id.toString() && (
            <div style={{ marginBottom: 12 }}>
              <input placeholder="Nombre del curso" value={newCursoName} onChange={(e) => setNewCursoName(e.target.value)} />
              <button type="button" onClick={() => handleAddCurso(sec.id)}>Agregar</button>
              <button type="button" onClick={() => { setAddingCursoTo(null); setNewCursoName(''); }}>Cancelar</button>
            </div>
          )}
          {sec.cursos.map((curso) => (
            <div key={curso.id} className="course-row">
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <button type="button" className="ab">Editar</button>
